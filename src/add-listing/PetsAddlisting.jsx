...
import { PetsListing } from "../../configs/schema";

function PetsAddListing() {
  ...
  const GetListDetails = async () => {
    try {
      const result = await db
        .select()
        .from(PetsListing)
        .where(eq(PetsListing.id, listid));

      setPetsInfo(result[0]);
  ...
  const validateForm = () => {
    const requiredFields = carDetails.petsDetails.filter(
      (item) => item.required
    );
        ...
        >
          <div>
            <h2 className="font-medium text-xl mb-6 text-gray-700">
              Enter Pet Listing Details
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {carDetails.petsDetails.map((item, index) => (
